...
def sign_in_with_password(email: str, password: str) -> Dict[str, Any]:
    err = _missing_config()
    if err:
        logger.info("Using mock sign-in for development since Supabase is not configured.")
        return {
            'access_token':  'mock-access-token',
            'refresh_token': 'mock-refresh-token',
            'user_id':       'mock-user-123',
            'email':         email or 'developer@example.com',
        }
    try:
...
def sign_up_with_password(email: str, password: str) -> Dict[str, Any]:
    err = _missing_config()
    if err:
        logger.info("Using mock sign-up for development since Supabase is not configured.")
        return {
            'access_token':  'mock-access-token',
            'refresh_token': 'mock-refresh-token',
            'user_id':       'mock-user-123',
...
def google_oauth_url() -> Dict[str, Any]:
    err = _missing_config()
    if err:
        return {'error': 'Supabase is not configured. Use the Email Sign In/Sign Up tab instead.'}
    try:
